...
import PolicyServices from "../../services/policies/Policy";
...
function Policy() {
  ...
  const fetch = async() => {
    setLoading(true)
  const res = await PolicyServices.getPolicies();
   setValue(res.data[0]);
   setTimeout(() => {
    setLoading(false)
   }, 500);
  ...
  const updatePolicy = async () => {
    const res = await PolicyServices.updatePolicy(newValue,value._id);
        ...
        {edit ? (
          <>
            ...
            <div className="text-end ">
              ...
              <button type="button"  onClick={updatePolicy}  class="btn    mx-2 btn-primary">
                Save Changes
              </button>
            </div>
          </>
